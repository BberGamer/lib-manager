...
<main class="fine-page">
    <div class="container fine-content fine-detail-page">
        ...
        <header class="fine-heading">
            <span><i class="fa-solid fa-receipt"></i> Khoản phạt #F<c:out value="${fine.id}" />
            </span>
            <h1>Chi tiết khoản phạt</h1><p>Thông tin đầy đủ về khoản phạt thư viện này</p></header>
        <section class="fine-detail-card">
            <div class="fine-detail-title">
                <h2><c:out value="${fine.borrowRecord.book.title}" /></h2>
                <span class="fine-badge ${fine.status == 'UNPAID' ? 'unpaid' : (fine.status == 'PAID' ? 'paid' : 'waived')}">
                    <c:choose><c:when test="${fine.status eq 'UNPAID'}">Chưa thanh toán</c:when>
                        <c:when test="${fine.status eq 'PAID'}">Đã thanh toán</c:when>
                        <c:otherwise>Đã miễn</c:otherwise></c:choose>
                        </span>
                    </div>
                    <dl>
                        <div>
                            <dt>Mã khoản phạt</dt>
                            <dd>#F<c:out value="${fine.id}" /></dd>
                </div>
                <div>
                    <dt>Lý do</dt>
                    <dd><c:out value="${fine.reason}" /></dd>
                </div>
                <div>
                    <dt>Số tiền</dt>
                ...
                <div>
                    <dt>Hạn trả</dt>
                    <dd><c:out value="${fine.borrowRecord.dueDate}" default="—" /></dd>
                </div>
                <div>
                    <dt>Ngày trả</dt>
                    <dd><c:out value="${fine.borrowRecord.returnDate}" default="—" /></dd>
                </div>
                <div>
                    <dt>Ngày tạo</dt>
                    <dd><c:out value="${fine.createdDate}" /></dd>
                </div>
                <div>
                    <dt>Ngày thanh toán</dt>
                    <dd><c:out value="${fine.paidDate}" default="—" /></dd>
                </div>
                <div>
                    <dt>Phương thức thanh toán</dt>
                    <dd><c:out value="${fine.paymentMethod}" default="—" /></dd>
                </div>
            </dl>
            <c:if test="${not empty fine.paymentNote}">
                <div class="fine-note">
                    <strong>Ghi chú thanh toán</strong>
                    <p><c:out value="${fine.paymentNote}" /></p>
                </div>
            </c:if>
        </section>
    </div>
