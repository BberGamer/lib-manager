...
<main class="borrow-page">
    ...
    <div class="container borrow-content">
        ...
        <section class="borrow-section" aria-labelledby="active-borrows-title">
            <h2 id="active-borrows-title"><i class="fa-solid fa-book-open"></i> Đang mượn</h2>
            <div class="borrow-table-wrap">
                <table class="borrow-table">
                    <thead><tr><th>Sách</th><th>Ngày mượn</th><th>Hạn trả</th><th>Gia hạn</th><th>Trạng thái</th><th>Thao tác</th></tr></thead>
                    <tbody>
                        <c:forEach var="record" items="${borrowPage.activeRecords}">
                            <tr>
                                ...
                                <td data-label="Hạn trả" class="${record.status eq 'OVERDUE' ? 'borrow-date-overdue' : ''}">
                                    ...
                                    <c:if test="${record.status eq 'OVERDUE'}"><small><i class="fa-solid fa-triangle-exclamation"></i> Quá hạn!</small></c:if>
                                </td>
                                <td data-label="Gia hạn"><c:out value="${record.renewalCount}" /> /
                                    <c:out value="${maximumRenewals}" /> lần</td>
                                ...
                                <td data-label="Thao tác">
                                    <c:choose>
                                        ...
                                        <c:otherwise><span class="renew-unavailable">Không thể gia hạn</span></c:otherwise>
                                    </c:choose>
                                </td>
                            </tr>
                        </c:forEach>
                        <c:if test="${empty borrowPage.activeRecords}">
                            <tr><td colspan="6" class="borrow-empty"><i class="fa-solid fa-book"></i><span>Bạn chưa có sách đang mượn.</span></td></tr>
                        </c:if>
                    </tbody>
                </table>
            </div>
        ...
        <section class="borrow-section borrow-history" aria-labelledby="borrow-history-title">
            <h2 id="borrow-history-title"><i class="fa-solid fa-clock-rotate-left"></i> Lịch sử mượn</h2>
            <div class="borrow-table-wrap">
                <table class="borrow-table">
                    ...
                    <tbody>
                        ...
                        <c:if test="${empty borrowPage.historyRecords}">
                            <tr><td colspan="4" class="borrow-empty"><i class="fa-solid fa-clock-rotate-left"></i><span>Chưa có lịch sử mượn sách.</span></td></tr>
                        </c:if>
                    </tbody>
                </table>
            </div>
