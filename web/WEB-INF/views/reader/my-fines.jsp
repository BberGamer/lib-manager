<%-- Trang danh sách khoản phạt của user, do MyFineServlet render từ GET /fine/my. Mong đợi request attributes finePage,
    selectedStatus, keyword, activePage và session loggedUser. --%>
                    ...
                    <%@ include file="/WEB-INF/views/fragments/header.jsp" %>
                        <main class="fine-page">
                            <div class="container fine-content">
                                ...
                                <c:if test="${not empty sessionScope.fineSuccessMessage}">
                                    <div class="fine-alert fine-alert-success">
                                        <i class="fa-solid fa-circle-check"></i>
                                        <span><c:out value="${sessionScope.fineSuccessMessage}" /></span>
                                    </div>
                                    <c:remove var="fineSuccessMessage" scope="session" />
                                </c:if>
                                <c:if test="${not empty sessionScope.fineErrorMessage}">
                                    <div class="fine-alert fine-alert-error">
                                        <i class="fa-solid fa-circle-exclamation"></i>
                                        <span><c:out value="${sessionScope.fineErrorMessage}" /></span>
                                    </div>
                                    <c:remove var="fineErrorMessage" scope="session" />
                                </c:if>
                                <section class="fine-summary" aria-label="Tổng quan khoản phạt">
                                    <article><span>Tổng số khoản phạt</span>
                                        <strong>
                                            <c:out value="${finePage.totalFines}" />
                                        </strong>
                                    </article>
                                    <article class="fine-summary-unpaid">
                                        <span>Số tiền chưa thanh toán</span>
                                        <strong>
                                            <fmt:formatNumber value="${finePage.unpaidAmount}" pattern="#,##0" /> VNĐ
                                        </strong>
                                    </article>
                                    <article class="fine-summary-paid">
                                        <span>Số tiền đã thanh toán</span>
                                        <strong>
                                            <fmt:formatNumber value="${finePage.paidAmount}" pattern="#,##0" /> VNĐ
                                        </strong>
                                    </article>
                                    <article>
                                        <span>Khoản phạt chưa thanh toán</span>
                                        <strong>
                                            <c:out value="${finePage.unpaidCount}" />
                                        </strong>
                                    </article>
                                </section>
                                <form class="fine-filters" action="${myFinesUrl}" method="get">
                                    <label>
                                        ...
                                        <select name="status">
                                            ...
                                            <option value="UNPAID" ${selectedStatus eq 'UNPAID' ? 'selected' : '' }>Chưa
                                                thanh toán</option>
                                            <option value="PAID" ${selectedStatus eq 'PAID' ? 'selected' : '' }>Đã thanh
                                                toán</option>
                                            <option value="WAIVED" ${selectedStatus eq 'WAIVED' ? 'selected' : '' }>Đã
                                                miễn</option>
                                        </select>
                                    </label>
                                    <label class="fine-search">
                                        <span>Tìm kiếm</span>
                                        <input name="keyword" value="${fn:escapeXml(keyword)}"
                                            placeholder="Tìm theo tên sách hoặc mã khoản phạt..."></label>
                                    <button type="submit">
                                ...
                                <c:choose>
                                    <c:when test="${not empty finePage.fines}">
                                                    ...
                                                    <c:forEach var="fine" items="${finePage.fines}">
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td data-label="Ngày mượn">
                                                                <c:out value="${fine.borrowRecord.borrowDate}"
                                                                    default="—" />
                                                            </td>
                                                            <td data-label="Hạn trả">
                                                                <c:out value="${fine.borrowRecord.dueDate}"
                                                                    default="—" />
                                                            </td>
                                                            <td data-label="Ngày trả">
                                                                <c:out value="${fine.borrowRecord.returnDate}"
                                                                    default="—" />
                                                            </td>
                                                            <td data-label="Chi tiết phạt">
                                                                <c:choose>
                                                                    <c:when test="${fine.fineType eq 'OVERDUE'}">
                                                                        Quá hạn
                                                                        <c:out value="${fine.overdueDays}" /> ngày
                                                                        (5.000 VNĐ/ngày)
                                                                    </c:when>
                                                                    <c:otherwise>
                                                            ...
                                                            <td data-label="Số tiền">
                                                                <strong>
                                                                    <fmt:formatNumber value="${fine.amount}"
                                                                        pattern="#,##0" />
                                                                    VNĐ
                                                                </strong>
                                                            </td>
                                                            <td data-label="Trạng thái">
                                                                <span
                                                                    ...
                                                                    <c:choose>
                                                                        <c:when test="${fine.status eq 'UNPAID'}">Chưa
                                                                            thanh toán</c:when>
                                                                        <c:when test="${fine.status eq 'PAID'}">Đã thanh
                                                                            toán</c:when>
                                    ...
                                    <c:otherwise>
                                        <section class="fine-empty">
                                            <i class="fa-solid fa-circle-check"></i>
                                            <h2>${empty keyword and selectedStatus eq 'ALL' ? 'Không có khoản phạt' :
                                                'Không tìm thấy khoản phạt phù hợp.'}
                                            </h2>
