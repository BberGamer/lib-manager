<%--
    Trang quản lý mượn trả do BorrowManagementServlet hiển thị.
    Mong đợi request attributes borrowList, borrowActionPrefix, totalPages, currentPageNum,
    selectedStatus và keyword; session attributes successMsg, errorMsg và loggedUser.
--%>
                ...
                <label>
                    ...
                    <select name="status">
                        ...
                        <option value="PENDING_PICKUP"
                                ${selectedStatus eq 'PENDING_PICKUP' ? 'selected' : ''}>Chờ nhận sách</option>
                        <option value="BORROWED"
                                ${selectedStatus eq 'BORROWED' ? 'selected' : ''}>Đang mượn</option>
                        <option value="RETURNED"
                                ${selectedStatus eq 'RETURNED' ? 'selected' : ''}>Đã trả</option>
                        <option value="OVERDUE"
                                ${selectedStatus eq 'OVERDUE' ? 'selected' : ''}>Quá hạn</option>
                        <option value="EXPIRED"
                                ${selectedStatus eq 'EXPIRED' ? 'selected' : ''}>Hết hạn nhận</option>
                        <option value="CANCELLED"
                                ${selectedStatus eq 'CANCELLED' ? 'selected' : ''}>Đã hủy</option>
                    </select>
                </label>
                        ...
                        <c:choose>
                            ...
                            <c:otherwise>
                                <c:forEach var="borrowRecord" items="${borrowList}">
                                    <tr>
                                        <td class="borrow-record-id"><c:out value="${borrowRecord.id}" /></td>
                                        <td>
                                            <strong><c:out value="${borrowRecord.user.fullName}" /></strong>
                                            <small>
                                        ...
                                        <td>
                                            <strong><c:out value="${borrowRecord.book.title}" /></strong>
                                            <small>ISBN: <c:out value="${borrowRecord.book.isbn}" /></small>
                                        </td>
                                        <td>
                                            <c:choose>
                                                <c:when test="${not empty borrowRecord.bookCopy}">
                                                    <span class="borrow-barcode">
                                                        <c:out value="${borrowRecord.bookCopy.barcode}" />
                                                    </span>
                                                </c:when>
                                                <c:otherwise>
                                                    <span class="borrow-muted">Chưa gán bản sao</span>
                                                </c:otherwise>
                                            </c:choose>
                                        </td>
                                        <td>
                                            <span><c:out value="${empty borrowRecord.borrowDate ? '-' : borrowRecord.borrowDate}" /></span>
                                            <small>Hạn: <c:out value="${empty borrowRecord.dueDate ? '-' : borrowRecord.dueDate}" /></small>
                                        </td>
                                        <td>
                                            <span class="borrow-status borrow-status-${fn:toLowerCase(borrowRecord.status)}">
                                                <c:choose>
                                                    <c:when test="${borrowRecord.status eq 'PENDING_PICKUP'}">Chờ nhận sách</c:when>
                                                    <c:when test="${borrowRecord.status eq 'BORROWED'}">Đang mượn</c:when>
                                                    <c:when test="${borrowRecord.status eq 'RETURNED'}">Đã trả</c:when>
                                                    <c:when test="${borrowRecord.status eq 'OVERDUE'}">Quá hạn</c:when>
                                                    <c:when test="${borrowRecord.status eq 'EXPIRED'}">Hết hạn nhận</c:when>
                                                    <c:when test="${borrowRecord.status eq 'CANCELLED'}">Đã hủy</c:when>
                                                    <c:otherwise><c:out value="${borrowRecord.status}" /></c:otherwise>
                                                </c:choose>
                                            </span>
                                        </td>
                                        <td class="borrow-table-actions">
                                            <c:choose>
                                                <c:when test="${borrowRecord.status eq 'PENDING_PICKUP'}">
                                                ...
                                                <c:when test="${borrowRecord.status eq 'BORROWED' || borrowRecord.status eq 'OVERDUE'}">
                                                    <div class="borrow-action-group">
                                                        ...
                                                        <c:if test="${not borrowRecord.hasFine}">
                                                            <button type="button" class="btn btn-sm borrow-fine-button"
                                                ...
                                                <c:otherwise><span class="borrow-muted">Hoàn tất</span></c:otherwise>
                                            </c:choose>
                                        </td>
                                    </tr>
                                </c:forEach>
        ...
        <c:if test="${totalPages gt 1}">
            <nav class="borrow-pagination" aria-label="Phân trang mượn sách">
                <c:if test="${currentPageNum gt 1}">
                    ...
                    <a href="${previousPageUrl}" aria-label="Trang trước"><i class="fa-solid fa-chevron-left"></i></a>
                </c:if>
                ...
                <c:if test="${currentPageNum lt totalPages}">
                    ...
                    <a href="${nextPageUrl}" aria-label="Trang sau"><i class="fa-solid fa-chevron-right"></i></a>
                </c:if>
            </nav>
        </c:if>
    </div>
...
<div class="borrow-modal" data-borrow-modal="loan" hidden>
    <section class="borrow-modal-dialog" role="dialog" aria-modal="true" aria-labelledby="loan-modal-title">
        ...
        <form action="${confirmPickupUrl}" method="post">
            ...
            <label><span>Độc giả</span><input type="text" readonly data-loan-field="readerName"></label>
            <label><span>Đầu sách mượn</span><input type="text" readonly data-loan-field="bookTitle"></label>
            <div class="borrow-modal-grid">
                <label><span>Mã yêu cầu</span><input type="text" readonly data-loan-field="requestId"></label>
                <label><span>Mã vạch bản sao</span><input class="borrow-monospace" type="text" readonly data-loan-field="barcode"></label>
                <label><span>Thời điểm yêu cầu</span><input type="text" readonly data-loan-field="requestDate"></label>
                <label><span>Hạn cuối nhận sách</span><input class="borrow-deadline" type="text" readonly data-loan-field="pickupDeadline"></label>
            </div>
            <div class="borrow-modal-actions">
                <button type="button" class="btn btn-secondary" data-close-borrow-modal>Hủy bỏ</button>
...
<div class="borrow-modal" data-borrow-modal="return" hidden>
    <section class="borrow-modal-dialog" role="dialog" aria-modal="true" aria-labelledby="return-modal-title">
        <h2 id="return-modal-title"><i class="fa-solid fa-rotate-left borrow-success-icon"></i> Nhận trả sách</h2>
        ...
        <form action="${confirmReturnUrl}" method="post">
            ...
            <label><span>Tên sách</span><input type="text" readonly data-return-field="bookTitle"></label>
            <label><span>Mã vạch</span><input class="borrow-monospace" type="text" readonly data-return-field="barcode"></label>
            <label>
            ...
            <label><span>Ghi chú bổ sung</span><textarea name="note" rows="3" placeholder="Tình trạng trang sách, ghi chú hao mòn..."></textarea></label>
            <div class="borrow-modal-actions">
                <button type="button" class="btn btn-secondary" data-close-borrow-modal>Hủy bỏ</button>
...
<div class="borrow-modal" data-borrow-modal="fine" hidden>
    <section class="borrow-modal-dialog" role="dialog" aria-modal="true" aria-labelledby="fine-modal-title">
        <h2 id="fine-modal-title"><i class="fa-solid fa-circle-dollar-to-slot borrow-danger-icon"></i> Lập phiếu phạt độc giả</h2>
        ...
        <form action="${createFineUrl}" method="post">
            ...
            <label><span>Độc giả bị phạt</span><input type="text" readonly data-fine-field="readerName"></label>
            <label><span>Tên đầu sách</span><input type="text" readonly data-fine-field="bookTitle"></label>
            <label>
                <span>Tình trạng cuốn sách</span>
            ...
            <label>
                ...
                <input type="number" name="amount" required min="1" step="1" data-fine-field="amount">
                <small>Hệ thống tự điền theo tình trạng sách; thủ thư có thể điều chỉnh khi cần.</small>
            </label>
            <label><span>Lý do phạt</span><input type="text" name="reason" required maxlength="255" placeholder="Mô tả tình trạng hỏng hoặc mất sách..."></label>
            <div class="borrow-modal-actions">
                <button type="button" class="btn btn-secondary" data-close-borrow-modal>Hủy bỏ</button>
