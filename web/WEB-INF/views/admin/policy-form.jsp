<%--
    Form thêm/sửa draft do AdminPolicyServlet hiển thị;
    Nhận policy, formMode, policyCategories và validationErrors.
    Phiên bản được backend quản lý tự động.
--%>
    ...
    <div class="policy-form-container">
        ...
        <section class="policy-form-card">
            <form method="post" action="${pageContext.request.contextPath}${actionPath}"
                ...
                <c:if test="${formMode == 'revision'}">
                    <input type="hidden" name="sourceId" value="${sourceId}">
                </c:if>
                <c:if test="${not empty validationErrors}">
                    <div class="policy-validation-summary" role="alert">
                ...
                <div class="policy-form-grid">
                    <label class="policy-field full ${not empty validationErrors.policyCode ? 'invalid' : ''}" data-policy-field>
                        ...
                        <span id="policy-code-hint" class="policy-field-hint">
                            Từ 2–50 ký tự: chữ in hoa, số hoặc dấu gạch dưới; bắt đầu bằng chữ.
                        </span>
                        <small id="policy-code-error" data-policy-error>
                            <c:out value="${validationErrors.policyCode}" />
                        </small>
                    </label>

                    <label class="policy-field full ${not empty validationErrors.title ? 'invalid' : ''}" data-policy-field>
                        <span>Tiêu đề *</span>
                        <input name="title" required minlength="3" maxlength="200" value="${fn:escapeXml(policy.title)}">
                    ...
                    <label class="policy-field full ${not empty validationErrors.category ? 'invalid' : ''}" data-policy-field>
                        ...
                        <select name="category" required>
                            <c:forEach var="item" items="${policyCategories}">
                                <option value="${item}" ${policy.category == item ? 'selected' : ''}>
                    ...
                    <label class="policy-field ${not empty validationErrors.effectiveFrom ? 'invalid' : ''}" data-policy-field>
                        ...
                        <input type="date" name="effectiveFrom" value="${not empty effectiveFromValue ? fn:escapeXml(effectiveFromValue) : policy.effectiveFrom}">
                    ...
                    <label class="policy-field ${not empty validationErrors.effectiveTo ? 'invalid' : ''}" data-policy-field>
                        ...
                        <input type="date" name="effectiveTo" value="${not empty effectiveToValue ? fn:escapeXml(effectiveToValue) : policy.effectiveTo}">
                    ...
                    <label class="policy-field full ${not empty validationErrors.content ? 'invalid' : ''}" data-policy-field>
                        <span>Nội dung *</span>
                        <textarea name="content" required maxlength="10000" rows="14" data-policy-content><c:out value="${policy.content}" /></textarea>
                ...
                <div class="policy-form-actions">
                    <a class="policy-button secondary" href="${listUrl}">Hủy</a>
                    <button class="policy-button primary" type="submit">
                        <i class="fa-solid fa-floppy-disk"></i> Lưu bản nháp
                    </button>
                </div>
            </form>
        </section>
