    ...
    <div class="container user-list-container">
        ...
        <div class="books-topbar user-topbar">
            <div class="results-info user-results-info">
                ...
                <% } else { %>
                    <i class="fa-solid fa-users fa-xs user-icon-primary-space"></i>
            ...
            <div class="user-topbar-actions">
                ...
                <div class="sort-group">
                    ...
                    %>
                        <a href="<%= ctx %><%= currentPath %>?q=<%= java.net.URLEncoder.encode(q,"UTF-8") %>&role=<%= roleFilter != null ? roleFilter : "" %>&active=<%= activeFilter != null ? activeFilter : "" %>&sort=<%= sf %>&order=<%= thisOrder %>&page=1"
        ...
        <div class="admin-card user-card">
            <div class="admin-section-head user-section-head">
                <h2 class="user-section-title">Danh sách người dùng</h2>
                <span class="user-section-count"><%= totalRecords %> bản ghi</span>
            </div>
                ...
                <table class="admin-table user-table">
                    ...
                    %>
                        <tr class="copy-row user-tr">
                            <td class="user-td-id"><%= u.getId() %></td>
                            <td class="user-td-username">
                                <a href="<%= ctx %>/user/profile?id=<%= u.getId() %>" class="user-username-link">
                                    <%= u.getUsername() %>
                                </a>
                            </td>
                            <td class="user-td-fullname"><%= u.getFullName() != null ? u.getFullName() : "—" %></td>
                            <td class="user-td-email"><%= u.getEmail() != null ? u.getEmail() : "—" %></td>
                            ...
                            <td class="user-td-status">
                                <% if (u.getActive() == 1) { %>
                                    <span class="user-status-badge-active">
                                        <i class="fa-solid fa-circle user-status-dot"></i> Active
                                    </span>
                                <% } else { %>
                                    <span class="user-status-badge-locked">
                                        <i class="fa-solid fa-circle user-status-dot"></i> Locked
                                    </span>
                                <% } %>
                            </td>
                            <td class="user-td-actions">
                                <div class="admin-row-actions user-row-actions">
                                    ...
                                    <form method="post" action="<%= ctx %><%= currentPath %>" class="user-action-form">
                                        ...
                                        <button type="submit" class="<%= u.getActive()==1 ? "user-action-lock" : "user-action-unlock" %>">
                        ...
                        <tr>
                            <td colspan="7" class="user-empty-td">
                                <div class="empty-state">
                                    ...
                                    <h3 class="user-empty-title">Không tìm thấy người dùng</h3>
                                    <p class="user-empty-desc">Hãy thử tìm kiếm với từ khóa khác.</p>
                                </div>
                            </td>
                        </tr>
                    <% } %>
                ...
                <nav aria-label="Phân trang">
                    <ul class="pagination user-pagination-ul">
                        ...
                        <li class="page-item <%= currentPageNum <= 1 ? "disabled" : "" %>">
                            <a class="page-link"
                               href="<%= ctx %><%= currentPath %>?q=<%= java.net.URLEncoder.encode(q,"UTF-8") %>&role=<%= roleFilter != null ? roleFilter : "" %>&active=<%= activeFilter != null ? activeFilter : "" %>&sort=<%= sortField %>&order=<%= sortOrder %>&page=<%= currentPageNum - 1 %>">
                           ...
                           if (totalPages <= 7) {
                               for (int pg = 1; pg <= totalPages; pg++) { %>
                                   <li class="page-item <%= pg == currentPageNum ? "active" : "" %>">
                                       <a class="page-link" href="<%= ctx %><%= currentPath %>?q=<%= java.net.URLEncoder.encode(q,"UTF-8") %>&role=<%= roleFilter != null ? roleFilter : "" %>&active=<%= activeFilter != null ? activeFilter : "" %>&sort=<%= sortField %>&order=<%= sortOrder %>&page=<%= pg %>"><%= pg %></a>
                                   </li>
                               <% }
                           } else {
                               // Show first 2 pages
    ...
    <div class="user-modal-content">
        ...
        <div class="user-modal-header">
            <h3 class="user-modal-title">
                <i class="fa-solid fa-user-plus user-modal-title-icon"></i> Tạo người dùng mới
            </h3>
            ...
            <div class="user-modal-form-grid">
                ...
                <div>
                    ...
                    <input id="cu_password" name="password" type="password" placeholder="Để trống = mặc định 'password'"
                           minlength="5" maxlength="100" class="form-control">
                </div>
                <div>
                ...
                <div>
                    ...
                    <input id="cu_studentId" name="studentId" placeholder="Ví dụ: HA123456"
